...
Version N: uses the SFNWC-PTOP products calculated with the 2016 version only which is available
at ICARE for Himawari from 11 July 2017
It was wronly assumed previously that this version is using the reprocessed data. 

Modif (January 2019): allows to select OPAQ clouds and to lower the truncation to a pressure level below 175 hPa
(not used so far)
...
if __name__ == '__main__': 
    ...
    parser.add_argument("-o","--opaq",type=str,choices=["y","n"],help="opaq (y) or not (n)")                   
    ...
    if 'ciclad' in socket.gethostname():
            ...
            if opaq: flexout = '/data/legras/flexout/STC/FORWN-OPAQ'
    ...
    while current_date <= date_end:
        ...
        if opaq:
            ptop_temp((cloud_flag & 2)==2) = np.ma.masked       
