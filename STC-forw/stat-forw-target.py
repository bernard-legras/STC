...
pile2_sav_stream = os.path.join(out_dir,'pile2-save-target-'+run_type+'-'+stream+'-h'+str(hmax)+'.pkl')
...
if quiet:
    # Output file
    print_file = os.path.join(out_dir,'out','pile-target-'+run_type+'-'+stream+'.out')
    fsock = open(print_file,'w')
    sys.stdout=fsock
    
print("stat-forw-target> process "+run_type+' '+stream)
...
for date in preDates[stream]:    
    ...
    print("stat-forw-target > date "+ date,np.sum(sources['veryhigh']),np.sum(sources['silviahigh']))
