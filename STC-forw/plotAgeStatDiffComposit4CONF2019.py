...
for supertype in supertypes:
    ...
    for date in dates:
        ...
        file_out = os.path.join(out_dir,'ageStatDiff-'+supertype+'-2017-'+date)
...
for hightype in hightypes:
    for supertype in ['EAZ','EAD','EIZ','EID']:
        ...
        # Kill all the data but the bin correponding to the source
        buf = result[hightype][supertype]['histog'][0,74]
        result[hightype][supertype]['histog'][0,:] = 0
        result[hightype][supertype]['histog'][0,74] = buf
...
#    hh = result['sh'][supertype]['histog'][0:249,:]
#    ssh = np.sum(hh,axis=0)
#    ssh[ssh==0]=1
#    hh = hh / ssh[np.newaxis,:]
#    thets = thetaxis[74:]
#    hhs = hh[:,74:]
#    ssv = np.sum(hhs,axis=1)
#    mmv = np.sum(hhs*thets[np.newaxis,:],axis=1)/ssv
#    vvv = np.sum(hhs*thets[np.newaxis,:]**2,axis=1)/ssv-mmv**2
#    time = np.arange(0,62.1,0.25)
#    [b,a] = np.polyfit(time[60:],vvv[60:],1)
#    print('diff '+supertype,0.5*b)
#    #plt.plot(time[1:],mmv[1:])
#    #plt.show()
#    plt.plot(time[60:],vvv[60:],time[60:],a+b*time[60:])
#    #plt.ylim(0,300)
#    plt.show()
...
for supertype in['EAD','EAZ','EID','EIZ']:
    ...
    [b,a] = np.polyfit(ageaxis[10:120],var2thet[10:120],1)
