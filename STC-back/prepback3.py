...
16 March 2018
Modified from prepback2 version which initializes on potential temperature surfaces but still keep the
wrong flag.
Flag changed to 0x3F (=63) which means fill grid + new parcel + relative time + theta coordinate
Modifications / prepback2:
    ENint = timedelta(hours=3)
    BACK-EIZ-FULL instead of EIZ
    target_range=np.array([[-30.,50.],[-179.,181.]])
    target_binx = 360; target_biny = 80                            
...
# now done in main
base_level = 100. # pressure in hPa not used here
# Thermodynamic constnat and reference level
R = 287.04
Cp = 1005.7
kappa = R/Cp
p0 = 100000.
...
if __name__ == '__main__': 
    ...
    if 'ciclad' in socket.gethostname():
            ...
            part_dir = os.path.join(flexout,'BACK-EIZ-FULL-'+date_beg.strftime('%b-%Y-')\
