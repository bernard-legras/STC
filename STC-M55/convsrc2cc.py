...
def main():
    ...
    parser.add_argument("-t","--tau",type=int,help="cloud cover time scale (h)")
    ...
    if quiet:
        # Output file
        print_file = os.path.join(out_dir,'out',platform+fdate.strftime('-%Y%m%d')+launch_number+'-'+advect+'-D01'+suffix+tauf+'.out')
    ...
    # Output file
    out_file = os.path.join(out_dir,platform+fdate.strftime('-%Y%m%d')+launch_number+'-'+advect+'-D01'+suffix+tauf+'.hdf5')
    ...
    prod0['src']['age'] = np.empty(shape=(nsrc,part0['numpart']),dtype='float')
...
@jit(nopython=True,cache=True)
def detrainer(itime, xi,yi,pi,ti,hyb,xf,yf, cc, idx_back,flag,ir_start,chi,passed,\
              xc,yc,pc,tc,age,\
              Lo1,La1,dlo,dla,cc_offset,tau):
    nhits = [0,0,0,0,0,0]
    ccmm = 0
...
#                     print('ll0 ',ll[j])
                #@@ test    
                ccm = cc[hyb[i],ll[j][1],ll[j][0]]
            ccm /= len(ll)
            ccmm = max(ccm,ccmm)
            # erode the parcel
            if ccm >= cc_offset:
                chi[i0] *= math.exp(-3600*tau*ccm)
                if passed[i0] >1:
                    if passed[i0] == 10:
                        if chi[i0] < 0.9:
                            xc[1,i0] = xi[i]
