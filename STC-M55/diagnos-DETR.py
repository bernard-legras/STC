...
source_range=np.array([[-10.,160.],[0.,50.]])
source_binx=77;source_biny=25
source_binx=170;source_biny=50
...
def main():
    ...
    if socket.gethostname() == 'graphium':
        STC_out = 'C:\\cygwin64\\home\\Bernard\\data\\STC\\STC-M55-OUT'
        ...
        STC_out = '/limbo/data/STC/STC-M55-DETR-OUT'
    ...
    for advect in ['EAD']:
        ...
        if src == 'src2' or full_detrain:
            convect = np.logical_and((prod0['flag_source'] & I_HIT) != 0,\
                                     (prod0['flag_source'] & I_DEAD) != 0)
            convect = np.logical_and(convect,(prod0['flag_source'] & I_CROSSED) == 0)                      
        elif src == 'src':
            convect = (prod0['flag_source'] & I_HIT) != 0
        else:
            print ('unknown src, what are you doing?')
            sys.exit()
...
#        plt.show()
        ...
        #@ test whether this si correct
        regionb = np.empty(len(region))
        for i in range(len(region)):
            regionb[i] = mask['mask'][idy[i],idx[i]]
        print('reg check ',np.min(region-regionb),np.max(region-regionb))        
        ...
        for reg in mask['regcode'].keys():
            if reg not in exreg:
                selec = np.logical_and(region==mask['regcode'][reg],convect)
