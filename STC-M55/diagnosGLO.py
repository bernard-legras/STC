...
import collections
from STCdata import STCinst, readAMES1001
...
def main():
    ...
    for date in [datetime(2017,8,4),]:

        # read the data
        filename = join(STC_out,date.strftime('BAL-%Y%m%d-OPZ-D01.pkl'))
        # read tdc file
        ...
        txt = date.strftime('%d %m %Y  ')+'convective prop {:5.1f} %'.format(100*prop0)
        ...
        if savefig: plt.savefig(date.strftime('figs/source-density-%Y%m%d.png'))
        ...
        # make a time vector
        # Quick fix that takes the true times from the data BAL files
        # which fits the numpart times from the backward output
        bal = readAMES1001(join(STC_out,'20170804UCSE.TXT'))
...
#        plt.show()
        ...
        ax2.set_title('mean age from source with contribution > 5%')

        if savefig: plt.savefig(date.strftime('figs/prop-n-age-%Y%m%d.png'))
        ...
        for reg in totreg.keys():
            if maxtotreg[reg] > 0.2:
                ...
                axes[ii].axis('on')
                axes[ii].set_title('age spectrum for region '+reg+' (log 10 #)')
                ii += 1
        if savefig: plt.savefig(date.strftime('figs/age-spectrum-%Y%m%d.png'))
