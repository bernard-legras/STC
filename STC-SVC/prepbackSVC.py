...
elif socket.gethostname() == 'ajikot-ThinkStation-P500':
    SVC_Dir = '/home/ajikot/data/SEL2'
    flexout = '/home/ajikot/sandbox'
    
part_dir = os.path.join(flexout,'BACK-SVC-EID-FULL-'+date_beg.strftime('%b-%Y-day%d-')+date_end.strftime('%d-D01'))
try:
    os.mkdir(part_dir)
except:
    pass        
...
# loop on the days for the satellite data
date_p = date_beg
while date_p <= date_end:
    # We go to the SEL2 directory containing orbits of the day
    try:
        ...
        continue
    # The files conrained in the directory are listed in ll
    ll = os.listdir()
    # loop on the orbits
    while len(ll)>0:
        file = ll.pop()
        ...
        # select part of the orbit in the FullAMA domain
        selec = (data['latitude']>0) & (data['latitude']<50) & (data['longitude']<160) & (data['longitude']>-10)
        # process only orbits that intersect the FullAMA domain
        if np.sum(selec)>0:
            # select the information for the part of the orbit in the domain
            ...
            # calculate the mean date of the orbit (as a datetime)
            ...
            #sect.x = sect1.x
            #sect.y = sect1.y
            # interpolate in time for the mean time
            # could be improved by replacing dtt by a vector for all times of the orbit (is it worth?) 
            for var in ['P','T','Z']:
                sect.var[var] = (1-dtt)*sect1.var[var] + dtt*sect2.var[var]
            del sect1
            del sect2
            # generate a list of interpolating functions (one for each profile)
            # For each profile, it provides a T and log(P) as a function of Z
            interpP = []
            interpT = []
            ...
            # loop on the layers (as the SEL2 product is provided as a list of layers)
            for layer in range(20):
                ...
                if np.sum(selec2) >0:
                    ...
                    for i in range(len(lons)):
                        ...
                        part0['tEbase'].append(interpTsel[i](cbase[i])) # initially wrong with ctop
        else:
            print('skipped orbit')
    date_p += timedelta(days=1)
